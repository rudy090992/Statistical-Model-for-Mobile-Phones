...
# Chapter 5: Estimating and predicting price
Now, after we have a better understanding of the data, we can start to build a model to predict the price of a mobile phone.

## Redefine the price level
...
strata_srs = data.frame(strata_srs, pw = rep(N/nrow(strata_srs), nrow(strata_srs)), fpc = c(rep(260, 36), rep(365, 50), rep(64, 9), rep(36, 5)))
# Estimate the population mean and its standard deviation
...
test_data <- mobile_dataset[-train_index, ]
# Select colunms for multiple linear regression
column_names_tree <- c("Price_level_av", "Rating", "No_of_ratings", "TotalReviwes", "RamSize_inGB", "RomSize_inGB", "Is_5G", "Company")
train_data <- train_data[column_names_tree]
test_data <- test_data[column_names_tree]

# column_names_tree <- c("Price_level_av", "Rating", "No_of_ratings", "TotalReviwes", "RamSize_inGB", "RomSize_inGB", "Is_5G", "Company")
# mobile_dataset_tree <- mobile_dataset[column_names_tree]
# Remove missing values from the dataset
mobile_dataset_tree <- mobile_dataset_tree %>%
        filter(!is.na(Price_level_av))
# Split the data into train and test sets
# set.seed(2023)
# train_data <- mobile_dataset_tree[sample(1:nrow(mobile_dataset_tree), 0.7*nrow(mobile_dataset_tree)), ]
# test_data <- mobile_dataset_tree[-sample(1:nrow(mobile_dataset_tree), 0.7*nrow(mobile_dataset_tree)), ]
...
library(tree)
tree_model <- tree(as.factor(Price_level_av) ~ ., data = train_data)
summary(tree_model)
```
```{r}
# Plot the tree
...
misclassification_rate <- mean(tree_pred != test_data$Price_level_av)
...
pruned_tree_pred <- predict(pruned_tree_model, test_data, type = "class")
misclassification_rate_pruned <- mean(pruned_tree_pred != test_data$Price_level_av)
print(paste("Misclassification rate:", misclassification_rate_pruned))
```
```{r}
library(caret)
# Create the confusion matrix
conf_matrix <- confusionMatrix(pruned_tree_pred, test_data$Price_level_av)
# Extract the accuracy, precision, and F1 score
accuracy <- conf_matrix$overall[1]
precision <- conf_matrix$byClass[1]
f1_score <- conf_matrix$byClass[5]
...
library(tidyr)
conf_matrix_df <- pivot_wider(conf_matrix_df, names_from = Reference, values_from = Freq)
# Save the confusion matrix as a csv file
# write.csv(conf_matrix_df, "/Users/berg/DataspellProjects/MDSA-UofC/DATA605/00_demos/confusion_matrix.csv")
