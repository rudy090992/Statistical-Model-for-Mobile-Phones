...
# Dataset and Cleanup (Done by Yongpeng Fu)
...
#  Chapter 4: Exploratory Data Analysis (Done by Yongpeng Fu)
...
#  Chapter 5: Statistical Data Analysis (Done by Yongpeng Fu)
...
First, let use explore the distribution for difference price level. From the following bar plot, you can see it is very close to uniformly distributed across the dataset.
...
```
### 5.1.1 Quadratic Discriminant Analysis (QDA)
Based on EDA in section 4.2, we know three numeric features have a great impact on price level a mobile phone belongs to, which are _Rating_, _RamSize_inGB_ and _RomSize_inGB_.
...
Then we fit the data into Quadratic Discriminant Analysis (QDA) as follows:
```{r 5.1.1.1, results='hold'}
...
```
After this, we are confident to apply this model on the testing dataset, and calculate the misclassification rate.
```{r 5.1.1.2, results='hold'}
cat(rep("-", 50), "\n")
cat("\nApply the fitted logistic regression model to the test set, And we can get the following confusion table.\n")
Prob.predict<-predict(Model.fit,test,type="response")
table(Prob.predict$class,test$Price_Level)
cat(rep("-", 50), "\n")
cat("The misclassification rate is: ", mean(Prob.predict$class != test$Price_Level))
```
...
### 5.1.2 Decision Tree (Classification)
...
```{r 5.1.2.2, results='hold'}
#Let us plot the tree as well
plot(tree.class)
text(tree.class, pretty=0)
```
To find the best tree size, we used cross-validation to select the "best" number of tree size (the terminal nodes). Based on the following plot, it seems tree size 9 gives the smallest deviance. Just to demonstrate how prunned tree works and also reduce the complecity of tree, we decide to use 8 as the "best" tree size.
```{r 5.1.2.3, results='hold'}
...
$$
```{r 5.1.3.1, warning=FALSE, results='hold'}
library(VGAM)
...
table(pred_fit_class, mobile_dataset$Price_Level)
cat(rep("-", 50), "\n")
cat("The misclassification rate is: ", mean(pred_fit_class != mobile_dataset$Price_Level))
```
